from ai_agents.toolboxs.ToolBox import ToolBox
...
class Agent:
	...
	def prepare_tools(self):
		"""
		Stores the tools in toolbox and returns their descriptions
	...
	def think(self, prompt):
		"""
		Runs the generate_text method on the model using the system prompt and tool descriptions.

		Parameters:
		prompt (str): The user query to generate a response for.

		Returns:
		dict: The response from the model as dictionary.
		"""
		...
		for tool in self.tools:
			if tool.__name__ == tool_choice:
				...
				if isinstance(response, dict):
					print(colored(json.dumps(response, indent=4), 'cyan'))
				else:
